#!/usr/bin/env python3
"""
HFP V2 Knowledge Distillation Training

Teacher (frozen standard Transformer) → Student (HFP architecture)
Loss = alpha * KL(student || teacher) + (1 - alpha) * CE(student, labels)

Usage:
    # Single GPU
    python train_distill.py \
        --teacher Qwen/Qwen2.5-1.5B \
        --dataset wikitext --dataset_config wikitext-103-raw-v1 \
        --seq_len 512 --batch_size 4 --epochs 3 \
        --output_dir ./checkpoints/hfp-qwen2.5-1.5b

    # Multi-GPU (DDP)
    torchrun --nproc_per_node=4 train_distill.py \
        --teacher Qwen/Qwen2.5-1.5B \
        --dataset wikitext --dataset_config wikitext-103-raw-v1 \
        --seq_len 512 --batch_size 2 --epochs 3 \
        --output_dir ./checkpoints/hfp-qwen2.5-1.5b

    # With custom HFP config
    python train_distill.py \
        --teacher meta-llama/Llama-3.2-1B \
        --freq_cutoff_ratio 0.125 --conv_kernel_size 7 \
        --dataset wikitext --dataset_config wikitext-103-raw-v1 \
        --output_dir ./checkpoints/hfp-llama-1b
"""

import argparse
import json
import logging
import math
import os
import time

import torch
import torch.nn.functional as F
import torch.distributed as dist
from torch.nn.parallel import DistributedDataParallel as DDP
from torch.utils.data import DataLoader, DistributedSampler
from transformers import AutoTokenizer

from hfp_distill.config import HFPConfig
from hfp_distill.model import HFPForCausalLM
from hfp_distill.loader import load_teacher_weights_into_student, load_teacher_for_distillation

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
logger = logging.getLogger(__name__)


# ---------------------------------------------------------------------------
# Dataset
# ---------------------------------------------------------------------------
class TextDataset(torch.utils.data.Dataset):
    """Tokenize text dataset into fixed-length chunks for LM training."""

    def __init__(self, tokenizer, dataset_name, dataset_config, split, seq_len, max_samples=None):
        from datasets import load_dataset

        logger.info(f"Loading dataset {dataset_name}/{dataset_config} split={split}")
        raw = load_dataset(dataset_name, dataset_config, split=split)

        # Concatenate all text and chunk into seq_len+1 blocks
        text_col = "text" if "text" in raw.column_names else raw.column_names[0]
        all_text = "\n\n".join(t for t in raw[text_col] if t.strip())

        logger.info(f"Tokenizing {len(all_text):,} characters ...")
        tokens = tokenizer.encode(all_text, add_special_tokens=False)
        logger.info(f"Got {len(tokens):,} tokens, chunking into seq_len={seq_len+1}")

        self.chunks = []
        for i in range(0, len(tokens) - seq_len, seq_len):
            self.chunks.append(tokens[i : i + seq_len + 1])  # +1 for label shift
            if max_samples and len(self.chunks) >= max_samples:
                break

        logger.info(f"Created {len(self.chunks):,} training samples")

    def __len__(self):
        return len(self.chunks)

    def __getitem__(self, idx):
        chunk = torch.tensor(self.chunks[idx], dtype=torch.long)
        return chunk[:-1], chunk[1:]  # input_ids, labels


# ---------------------------------------------------------------------------
# Loss
# ---------------------------------------------------------------------------
def distillation_loss(
    student_logits: torch.Tensor,   # [B, N, V]
    teacher_logits: torch.Tensor,   # [B, N, V]
    labels: torch.Tensor,           # [B, N]
    temperature: float = 2.0,
    alpha: float = 0.5,
) -> tuple[torch.Tensor, dict]:
    """
    L = alpha * T^2 * KL(student_soft || teacher_soft) + (1-alpha) * CE(student, labels)
    """
    V = student_logits.size(-1)

    # KL divergence on soft targets
    student_log_probs = F.log_softmax(student_logits / temperature, dim=-1)
    teacher_probs = F.softmax(teacher_logits / temperature, dim=-1)
    kl = F.kl_div(
        student_log_probs.view(-1, V),
        teacher_probs.view(-1, V),
        reduction="batchmean",
    ) * (temperature ** 2)

    # Cross-entropy on hard labels
    ce = F.cross_entropy(student_logits.view(-1, V), labels.view(-1))

    loss = alpha * kl + (1.0 - alpha) * ce
    return loss, {"kl": kl.item(), "ce": ce.item(), "loss": loss.item()}


# ---------------------------------------------------------------------------
# Training
# ---------------------------------------------------------------------------
def train(args):
    # ---- Distributed setup ----
    ddp = int(os.environ.get("RANK", -1)) != -1
    if ddp:
        dist.init_process_group("nccl")
        rank = dist.get_rank()
        local_rank = int(os.environ["LOCAL_RANK"])
        world_size = dist.get_world_size()
        device = f"cuda:{local_rank}"
        torch.cuda.set_device(device)
    else:
        rank = 0
        local_rank = 0
        world_size = 1
        device = "cuda" if torch.cuda.is_available() else "cpu"

    is_main = rank == 0
    dtype = torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16

    # ---- Tokenizer ----
    tokenizer = AutoTokenizer.from_pretrained(args.teacher, trust_remote_code=True)
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token

    # ---- Build student ----
    if is_main:
        logger.info("Building HFP student model ...")
    config = HFPConfig.from_pretrained(
        args.teacher,
        freq_cutoff_ratio=args.freq_cutoff_ratio,
        conv_kernel_size=args.conv_kernel_size,
        dtype=dtype,
    )
    student = HFPForCausalLM(config).to(dtype).to(device)

    # Load teacher weights into student (attention → HFP, MLP/embed copied)
    if is_main:
        logger.info("Loading teacher weights into student ...")
    load_teacher_weights_into_student(student, args.teacher, dtype=dtype)
    student = student.to(device)

    if is_main:
        params = student.count_parameters()
        logger.info(
            f"Student: {params['total']/1e6:.1f}M total, "
            f"{params['hfp_specific']/1e6:.1f}M HFP-specific"
        )

    # ---- Load frozen teacher ----
    if is_main:
        logger.info("Loading frozen teacher model ...")
    teacher = load_teacher_for_distillation(args.teacher, dtype=dtype, device=device)

    # ---- Dataset ----
    dataset = TextDataset(
        tokenizer, args.dataset, args.dataset_config,
        split=args.split, seq_len=args.seq_len, max_samples=args.max_samples,
    )
    sampler = DistributedSampler(dataset, shuffle=True) if ddp else None
    dataloader = DataLoader(
        dataset,
        batch_size=args.batch_size,
        shuffle=(sampler is None),
        sampler=sampler,
        num_workers=args.num_workers,
        pin_memory=True,
        drop_last=True,
    )

    # ---- DDP wrap ----
    if ddp:
        student = DDP(student, device_ids=[local_rank])

    # ---- Optimizer & Scheduler ----
    # Separate HFP-specific params for potentially different LR
    hfp_params = []
    other_params = []
    raw_student = student.module if ddp else student
    for name, p in raw_student.named_parameters():
        if not p.requires_grad:
            continue
        if any(k in name for k in ["dw_conv", "pw_conv", "gate_linear", "alpha_raw"]):
            hfp_params.append(p)
        else:
            other_params.append(p)

    optimizer = torch.optim.AdamW(
        [
            {"params": other_params, "lr": args.lr},
            {"params": hfp_params, "lr": args.lr_hfp},
        ],
        weight_decay=args.weight_decay,
        betas=(0.9, 0.95),
    )

    total_steps = len(dataloader) * args.epochs
    warmup_steps = min(args.warmup_steps, total_steps // 10)

    def lr_schedule(step):
        if step < warmup_steps:
            return step / max(warmup_steps, 1)
        progress = (step - warmup_steps) / max(total_steps - warmup_steps, 1)
        return 0.5 * (1.0 + math.cos(math.pi * progress))

    scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_schedule)

    scaler = torch.amp.GradScaler("cuda", enabled=(dtype == torch.float16))

    # ---- Training loop ----
    os.makedirs(args.output_dir, exist_ok=True)

    global_step = 0
    best_loss = float("inf")

    for epoch in range(args.epochs):
        if ddp:
            sampler.set_epoch(epoch)
        student.train()
        epoch_loss = 0.0
        epoch_start = time.time()

        for batch_idx, (input_ids, labels) in enumerate(dataloader):
            input_ids = input_ids.to(device)
            labels = labels.to(device)

            # Teacher forward (no grad)
            with torch.no_grad():
                teacher_out = teacher(input_ids)
                teacher_logits = teacher_out.logits if hasattr(teacher_out, "logits") else teacher_out

            # Student forward
            with torch.amp.autocast("cuda", dtype=dtype):
                student_logits = student(
                    input_ids,
                    use_gradient_checkpointing=args.gradient_checkpointing,
                )
                loss, metrics = distillation_loss(
                    student_logits, teacher_logits, labels,
                    temperature=args.temperature,
                    alpha=args.alpha,
                )

            # Backward
            scaler.scale(loss).backward()

            if args.max_grad_norm > 0:
                scaler.unscale_(optimizer)
                torch.nn.utils.clip_grad_norm_(student.parameters(), args.max_grad_norm)

            scaler.step(optimizer)
            scaler.update()
            optimizer.zero_grad(set_to_none=True)
            scheduler.step()

            epoch_loss += metrics["loss"]
            global_step += 1

            # Logging
            if is_main and global_step % args.log_every == 0:
                avg_loss = epoch_loss / (batch_idx + 1)
                lr_curr = scheduler.get_last_lr()[0]
                elapsed = time.time() - epoch_start
                samples_per_sec = (batch_idx + 1) * args.batch_size * world_size / elapsed
                logger.info(
                    f"Epoch {epoch+1}/{args.epochs} "
                    f"Step {global_step}/{total_steps} "
                    f"Loss={avg_loss:.4f} KL={metrics['kl']:.4f} CE={metrics['ce']:.4f} "
                    f"LR={lr_curr:.2e} "
                    f"Speed={samples_per_sec:.1f} samples/s"
                )

            # Save checkpoint
            if is_main and global_step % args.save_every == 0:
                _save_checkpoint(raw_student, optimizer, config, global_step, args)

        # Epoch summary
        avg_epoch_loss = epoch_loss / len(dataloader)
        if is_main:
            elapsed = time.time() - epoch_start
            logger.info(
                f"Epoch {epoch+1} done — Avg Loss={avg_epoch_loss:.4f} "
                f"Time={elapsed/60:.1f}min"
            )
            if avg_epoch_loss < best_loss:
                best_loss = avg_epoch_loss
                _save_checkpoint(raw_student, optimizer, config, global_step, args, tag="best")

    # Final save
    if is_main:
        _save_checkpoint(raw_student, optimizer, config, global_step, args, tag="final")
        logger.info(f"Training complete. Best loss: {best_loss:.4f}")

    if ddp:
        dist.destroy_process_group()


def _save_checkpoint(model, optimizer, config, step, args, tag=None):
    name = f"checkpoint-{tag}" if tag else f"checkpoint-step{step}"
    path = os.path.join(args.output_dir, name)
    os.makedirs(path, exist_ok=True)

    torch.save(model.state_dict(), os.path.join(path, "model.pt"))
    torch.save(optimizer.state_dict(), os.path.join(path, "optimizer.pt"))
    torch.save({"step": step}, os.path.join(path, "training_state.pt"))

    # Save HFP config
    config_dict = {k: v for k, v in config.__dict__.items() if k != "dtype"}
    config_dict["dtype"] = str(config.dtype)
    with open(os.path.join(path, "hfp_config.json"), "w") as f:
        json.dump(config_dict, f, indent=2)

    logger.info(f"Saved checkpoint to {path}")


# ---------------------------------------------------------------------------
# CLI
# ---------------------------------------------------------------------------
def parse_args():
    p = argparse.ArgumentParser(description="HFP V2 Distillation Training")

    # Model
    p.add_argument("--teacher", type=str, required=True,
                   help="HuggingFace teacher model name/path (e.g., Qwen/Qwen2.5-1.5B)")

    # HFP config
    p.add_argument("--freq_cutoff_ratio", type=float, default=0.125,
                   help="Low/high frequency split ratio (default: 1/8)")
    p.add_argument("--conv_kernel_size", type=int, default=7,
                   help="Depthwise conv kernel size for high-freq (default: 7)")

    # Data
    p.add_argument("--dataset", type=str, default="wikitext",
                   help="HuggingFace dataset name")
    p.add_argument("--dataset_config", type=str, default="wikitext-103-raw-v1",
                   help="Dataset configuration name")
    p.add_argument("--split", type=str, default="train")
    p.add_argument("--seq_len", type=int, default=512,
                   help="Training sequence length")
    p.add_argument("--max_samples", type=int, default=None,
                   help="Max number of training samples (for debugging)")

    # Training
    p.add_argument("--batch_size", type=int, default=4)
    p.add_argument("--epochs", type=int, default=3)
    p.add_argument("--lr", type=float, default=2e-5,
                   help="LR for pretrained weights (embed, MLP, attention projections)")
    p.add_argument("--lr_hfp", type=float, default=1e-4,
                   help="LR for HFP-specific weights (conv, gate, alpha) — higher for cold-start")
    p.add_argument("--weight_decay", type=float, default=0.01)
    p.add_argument("--max_grad_norm", type=float, default=1.0)
    p.add_argument("--warmup_steps", type=int, default=200)
    p.add_argument("--temperature", type=float, default=2.0,
                   help="Distillation temperature")
    p.add_argument("--alpha", type=float, default=0.5,
                   help="Loss weight: alpha*KL + (1-alpha)*CE")
    p.add_argument("--gradient_checkpointing", action="store_true",
                   help="Enable gradient checkpointing to reduce memory")

    # Output
    p.add_argument("--output_dir", type=str, default="./checkpoints/hfp-distill")
    p.add_argument("--log_every", type=int, default=10)
    p.add_argument("--save_every", type=int, default=500)
    p.add_argument("--num_workers", type=int, default=4)

    return p.parse_args()


if __name__ == "__main__":
    train(parse_args())
